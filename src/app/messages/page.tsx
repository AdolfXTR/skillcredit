  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`messages-${profile.id}`)
      .on("postgres_changes", {
        ...
        if (activeConvo && msg.sender_id === activeConvo.id) {
  ...
  async function sendMessage() {
    ...
    try {
      await supabase.from("notifications").insert({
        ...
        body: imageUrl ? "📷 Sent a photo" : newMsg.trim().slice(0, 80),
        link: "/messages",
      });
    } catch (_) {}
            ...
            {conversations.length === 0 ? (
              ...
              <div key={c.other_user.id}
                ...
                <div style={{ position: "relative", flexShrink: 0 }}>
                  <div style={{ width: 40, height: 40, borderRadius: "50%", background: LEVEL_COLORS[c.other_user.level] || "#2d6a4f", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", height: "100%", animation: "fadeIn 0.15s ease", background: "#fff" }}>
              <div style={{ padding: "18px 22px", borderBottom: "1.5px solid #f0ece4", flexShrink: 0 }}>
                ...
                <input value={searchUser} onChange={e => handleSearchUsers(e.target.value)} placeholder="Search by name or @username..." autoFocus
                  style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "1.5px solid #e8e2d9", background: "#faf8f4", color: "#333", fontSize: 14, fontFamily: "'DM Sans', sans-serif", transition: "border-color 0.15s" }}
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: "10px 14px" }}>
                ...
                {searchResults.map(user => (
                  <div key={user.id} className="convo-item" onClick={() => openConversation(user)}
                    ...
                    <div style={{ width: 42, height: 42, borderRadius: "50%", background: LEVEL_COLORS[user.level] || "#2d6a4f", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
          ...
          {!showNewChat && !activeConvo && (
            <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12 }}>
              ...
              <button onClick={() => setShowNewChat(true)}
                style={{ marginTop: 6, padding: "10px 24px", borderRadius: 10, background: "#2d6a4f", color: "#fff", border: "none", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
                + New Message
              </button>
            </div>
          )}
            ...
            <>
              ...
              <div style={{ padding: "12px 20px", borderBottom: "1.5px solid #e8e2d9", display: "flex", alignItems: "center", gap: 12, flexShrink: 0, background: "#fff" }}>
                <div style={{ position: "relative" }}>
                  <div style={{ width: 38, height: 38, borderRadius: "50%", background: LEVEL_COLORS[activeConvo.level] || "#2d6a4f", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff" }}>
                ...
                <a href="/listings" style={{ padding: "6px 14px", borderRadius: 8, background: "#f5f0e8", color: "#555", fontSize: 12, fontWeight: 700, border: "1.5px solid #e8e2d9", transition: "all 0.12s" }}
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px 8px" }}>
                {messages.length === 0 && (
                  <div style={{ textAlign: "center", paddingTop: "18vh" }}>
                    <div style={{ width: 50, height: 50, borderRadius: "50%", background: LEVEL_COLORS[activeConvo.level] || "#2d6a4f", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, fontWeight: 700, color: "#fff", margin: "0 auto 12px" }}>
                      {getInitials(activeConvo.full_name)}
                    </div>
                    ...
                    {group.messages.map((msg, i) => {
                      ...
                      return (
                        <div key={msg.id} className="msg-in"
                          ...
                          {!isMe ? (
                            showAvatar ? (
                              <div style={{ width: 26, height: 26, borderRadius: "50%", background: LEVEL_COLORS[activeConvo.level] || "#2d6a4f", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
                                {getInitials(activeConvo.full_name)}
                              </div>
                          ...
                          <div style={{ maxWidth: "65%", display: "flex", flexDirection: "column", alignItems: isMe ? "flex-end" : "flex-start" }}>
                            {msg.message_type === "image" && msg.image_url && (
                              <img src={msg.image_url} alt="img" className="img-msg"
                            ...
                            {msg.content && (
                              <div style={{
                                padding: "9px 13px",
                                borderRadius: isMe
                                  ? sameSenderPrev ? "16px 4px 4px 16px" : "16px 16px 4px 16px"
                                  : sameSenderPrev ? "4px 16px 16px 4px" : "4px 16px 16px 16px",
              ...
              {imagePreview && (
                <div style={{ padding: "10px 18px", background: "#e8f4e8", borderTop: "1.5px solid #c6e8d4", display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <button onClick={() => { setImagePreview(null); setImageFile(null); }}
                    style={{ background: "#fce8e8", border: "1.5px solid #fca5a5", borderRadius: 7, color: "#dc2626", fontSize: 12, fontWeight: 600, padding: "4px 10px", cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
                    Remove
                  </button>
                </div>
              )}
              ...
              {showEmoji && (
                <div style={{ padding: "10px 14px", background: "#fff", borderTop: "1.5px solid #e8e2d9", display: "flex", flexWrap: "wrap", gap: 2, animation: "slideUp 0.12s ease" }}>
                  {EMOJI_LIST.map(emoji => (
                    <span key={emoji} className="emoji-btn" onClick={() => { setNewMsg(prev => prev + emoji); textareaRef.current?.focus(); }}>
                      {emoji}
                    </span>
                  ))}
                </div>
              ...
              <div style={{ padding: "10px 14px", borderTop: "1.5px solid #e8e2d9", background: "#fff", display: "flex", gap: 7, alignItems: "flex-end", flexShrink: 0 }}>
                ...
                <button className="tool-btn" onClick={() => setShowEmoji(p => !p)} title="Emoji"
                  style={{ background: showEmoji ? "#e8f4e8" : "#f5f0e8", borderColor: showEmoji ? "#c6e8d4" : "#e8e2d9" }}>
                  😊
                </button>
