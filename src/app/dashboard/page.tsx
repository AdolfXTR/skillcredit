...
// ── FIXED getBadgeTier ────────────────────────────────────────────────────────
// OLD BUG: every tier used && so 5000 XP + 0 sessions = Seedling
// FIX: XP drives advancement; sessions/rating only gate upper tiers
const BADGE_TIERS = [
          ...
          <div style={{ position:"relative" }} onClick={e=>{ e.stopPropagation(); setShowMenu(m=>!m); }}>
            ...
            {showMenu && (
              <div style={{ position:"absolute", right:0, top:44, background:"#fff", border:"1.5px solid #e8e2d9", borderRadius:18, padding:8, width:210, boxShadow:"0 16px 48px rgba(0,0,0,0.15)", zIndex:200 }}>
                <div style={{ padding:"10px 12px 12px", borderBottom:"1px solid #f0ece4", marginBottom:6 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                    ...
                    <div>
                      ...
                      {/* ✅ FIXED: only show once, no duplicate */}
                ...
                {[["👤","My Profile","/profile"],["👥","People","/people"],["📋","Create Listing","/listings/create"],["✅","Get Verified","/verify"],["💰","Wallet","/wallet"],["🏆","Leaderboard","/leaderboard"],["🔔","Notifications","/notifications"]].map(([icon,label,href])=>(
      ...
      <div style={{ maxWidth:1200, margin:"0 auto", padding:"28px" }}>
        ...
        <div className="card" style={{ padding:"26px 30px", marginBottom:16, display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:20, animation:"fadeUp 0.4s ease", position:"relative", overflow:"hidden" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:16 }}>
            <div style={{ position:"relative" }}>
              ...
              <div style={{ position:"absolute", bottom:-2, right:-2, background:"#fff", borderRadius:"50%", padding:2, fontSize:14, lineHeight:1 }}>{LEVEL_ICONS[levelInfo.name]}</div>
            </div>
            <div>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:7, flexWrap:"wrap" }}>
                ...
                {/* ✅ FIXED: Badge tier pill — separate from XP level, shown ONCE here only */}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 320px", gap:16, animation:"fadeUp 0.4s 0.13s ease both" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            {/* BADGE CARD — ✅ FIXED: no duplicate name rendering */}
            <div className="card" style={{ padding:"20px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
                ...
                {/* This is the ONLY place badge name appears */}
              ...
              {/* Big badge display — emoji + desc only, NO name repeated */}
              ...
              {nextBadge && (
                <div>
                  ...
                  {[
                    ...
                    const pct  = r.req > 0 ? Math.min((r.current / r.req) * 100, 100) : 100;
                    return (
            ...
            <div className="card" style={{ padding:"20px" }}>
              ...
              {[
                { icon:"⭐", label:"Rating",   pts:ratingPts,  max:80, detail:`${avgRating.toFixed(1)} avg × ${sessions}`,          color:"#f59e0b" },
                { icon:"📚", label:"Sessions", pts:sessionPts, max:15, detail:`${sessions} sessions × 2`,                            color:"#2d6a4f" },
                { icon:"🔄", label:"Repeats",  pts:repeatPts,  max:10, detail:`${repeatClients} repeat clients × 5`,                color:"#6366f1" },
                { icon:"⚠️", label:"Disputes", pts:disputePts, max:0,  detail:disputes===0?"No disputes ✓":`${disputes} × -15`,  color:disputes>0?"#dc2626":"#aaa" },
              ].map(r=>(
