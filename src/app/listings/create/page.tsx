  ...
  return (
    <div>
      ...
      {preview ? (
        <div className="relative rounded-2xl overflow-hidden border-2 border-emerald-400 bg-stone-100">
          ...
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
            <button onClick={() => { setPreview(null); setUploaded(false); }}
              className="px-4 py-2 bg-white text-stone-800 rounded-xl text-sm font-bold border-0 cursor-pointer">
              Change Image
            </button>
          </div>
          {uploading && (
          ...
          {uploaded && !uploading && (
            <div className="absolute top-3 right-3 bg-emerald-600 text-white text-xs font-black px-2.5 py-1 rounded-full">
              ✓ Uploaded
            </div>
          )}
        </div>
        ...
        <div
          className="border-2 border-dashed border-stone-300 rounded-2xl h-48 flex flex-col items-center justify-center cursor-pointer hover:border-emerald-400 hover:bg-emerald-50 transition-all"
          onDrop={handleDrop}
      ...
      <div className="flex flex-col gap-3">
        {items.map((item, idx) => (
          <div key={idx} className="bg-stone-50 rounded-2xl border border-stone-200 p-3 flex gap-3 items-start">
            {item.type === "image" && (
              <img src={item.url} alt={item.name} className="w-20 h-16 object-cover rounded-xl flex-shrink-0" />
            )}
            {item.type === "video" && (
              <div className="w-20 h-16 rounded-xl bg-stone-200 flex items-center justify-center flex-shrink-0 text-2xl">🎬</div>
            )}
            {item.type === "doc" && (
              <div className="w-20 h-16 rounded-xl bg-stone-200 flex items-center justify-center flex-shrink-0 text-2xl">📄</div>
            )}
            ...
            <button onClick={() => removeItem(idx)}
              className="text-stone-400 hover:text-red-500 border-0 bg-transparent cursor-pointer text-lg leading-none flex-shrink-0">
              ×
            </button>
          </div>
        ))}
...
export default function CreateListingPage() {
  ...
  const selectedSkill = isCustomSkill
    ? { name: customSkillName, category: customSkillCat || "Other" }
  ...
  const handleSkillChange = (val: string) => {
    if (val === OTHER_SKILL_ID) {
      ...
      setCustomSkillName(""); setCustomSkillCat("");
  ...
  const resetForm = () => {
    ...
    setForm({ skill_id: "", title: "", description: "", prerequisites: "", outcomes: "", materials: "", format: "", duration: 60, credit_price: 10 });
    setError("");
  };
      ...
      <div className="bg-white rounded-3xl border border-stone-200 p-12 text-center max-w-md shadow-lg w-full mx-4">
        {thumbnailUrl && (
          <div className="w-full h-40 rounded-2xl overflow-hidden mb-6">
            <img src={thumbnailUrl} alt="listing" className="w-full h-full object-cover" />
          </div>
        )}
        ...
        <div className="flex gap-3 mb-3">
          {doneListingId && (
            <a href={`/listings/${doneListingId}`} className="flex-1 py-3 bg-stone-100 text-stone-700 rounded-xl text-sm font-bold no-underline text-center hover:bg-stone-200 transition-colors">
              View Listing →
            </a>
          )}
          <a href="/listings" className="flex-1 py-3 bg-emerald-600 text-white rounded-xl text-sm font-bold no-underline text-center hover:bg-emerald-700 transition-colors">
            Browse All →
          </a>
        </div>
        <button onClick={resetForm} className="w-full py-3 bg-transparent text-stone-400 border border-stone-200 rounded-xl text-sm font-semibold cursor-pointer hover:bg-stone-50 transition-colors">
        ...
        <div className="bg-white rounded-3xl border border-stone-200 p-8 shadow-sm">
          ...
          {step === 2 && (
            <div className="flex flex-col gap-5">
              ...
              {userId && (
                <PortfolioUploader userId={userId} onChange={setPortfolioItems} />
              )}
              {portfolioItems.length === 0 && (
                <p className="text-xs text-stone-400 text-center">You can skip this and add samples later.</p>
              )}
            </div>
          )}
