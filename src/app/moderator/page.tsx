...
const REMOVAL_REASONS = [
  ...
  "Threatening or dangerous content",
...
export default function ModeratorDashboard() {
  const [tab, setTab] = useState<"disputes" | "content" | "listings" | "ratings" | "reports">("disputes");
  ...
  async function loadAll() {
    ...
    setModProfile(prof);
    await Promise.all([loadDisputes(), loadListings(), loadRatings(), loadReports(), loadAllContent()]);
    setLoading(false);
  }
  ...
  async function loadAllContent() {
    ...
    async function fetchAuthors(ids: string[]) {
      ...
      const { data } = await supabase.from("profiles")
        .select("id, full_name, username, level, avatar_url")
        .in("id", [...new Set(ids)]);
      return data || [];
    }
    ...
    (posts || []).forEach((p: any) => items.push({
      ...
      author: postAuthors.find((a: any) => a.id === p.author_id),
    ...
    (answers || []).forEach((c: any) => {
      items.push({
        id: c.id, type: "comment", content: c.content, author_id: c.author_id,
        author: commentAuthors.find((a: any) => a.id === c.author_id),
    ...
    (bounties || []).forEach((b: any) => items.push({
      id: b.id, type: "bounty", content: b.description || b.title, author_id: b.poster_id,
      author: bountyAuthors.find((a: any) => a.id === b.poster_id),
      created_at: b.created_at, status: b.status, extra: b.title,
    ...
    (allListings || []).forEach((l: any) => items.push({
      id: l.id, type: "listing", content: l.description || l.title, author_id: l.teacher_id,
      author: listingAuthors.find((a: any) => a.id === l.teacher_id),
      created_at: l.created_at, status: l.is_active ? "active" : "inactive",
      extra: `${l.title} · ${l.credit_price} cr · ${l.format}`,
    ...
    (profileBios || []).forEach((p: any) => items.push({
      id: `bio-${p.id}`, type: "bio", content: p.bio, author_id: p.id,
      author: { id: p.id, full_name: p.full_name, username: p.username, level: p.level, avatar_url: p.avatar_url },
    ...
    (usernames || []).forEach((p: any) => items.push({
      id: `username-${p.id}`, type: "username", content: p.username, author_id: p.id,
      author: { id: p.id, full_name: p.full_name, username: p.username, level: p.level, avatar_url: p.avatar_url },
      created_at: p.created_at,
    }));
    ...
    (sessionNotes || []).forEach((s: any) => items.push({
      id: `snote-${s.id}`, type: "session_note", content: s.notes, author_id: s.teacher_id,
      author: snoteAuthors.find((a: any) => a.id === s.teacher_id),
  ...
  async function loadDetail(item: ContentItem) {
    ...
    if (item.type === "post") {
      const { data: comments } = await supabase.from("forum_answers")
        ...
        .eq("post_id", item.id).order("created_at", { ascending: true }).limit(30);
  ...
  async function handleRemoveContent() {
    ...
    try {
      switch (item.type) {
        case "post":
          await supabase.from("forum_posts").update({ status: "removed", body: "[Removed by moderator]" }).eq("id", item.id);
          break;
        case "comment":
          await supabase.from("forum_answers").delete().eq("id", item.id);
          break;
        case "listing":
          await supabase.from("listings").update({ is_active: false }).eq("id", item.id);
          break;
        case "bounty":
          await supabase.from("bounties").update({ status: "removed" }).eq("id", item.id);
          break;
        case "bio":
          await supabase.from("profiles").update({ bio: "" }).eq("id", item.author_id);
          break;
        case "avatar":
        ...
        case "username":
          ...
          break;
        case "session_note":
          await supabase.from("sessions").update({ notes: "" }).eq("id", item.id.replace("snote-", ""));
          break;
      }
  ...
  async function handleResolveDispute() {
    ...
    if (resolution === "teacher") {
      await supabase.rpc("increment_credits", { user_id: s.teacher_id, amount: s.credit_amount });
      await supabase.from("escrow").update({ status: "released" }).eq("session_id", s.id);
    } else if (resolution === "learner") {
      await supabase.rpc("increment_credits", { user_id: s.learner_id, amount: s.credit_amount });
      await supabase.from("escrow").update({ status: "refunded" }).eq("session_id", s.id);
    } else {
      const half = Math.floor(s.credit_amount / 2);
      await supabase.rpc("increment_credits", { user_id: s.teacher_id, amount: half });
      await supabase.rpc("increment_credits", { user_id: s.learner_id, amount: s.credit_amount - half });
      await supabase.from("escrow").update({ status: "released" }).eq("session_id", s.id);
    }
    await supabase.from("sessions").update({ status: "completed" }).eq("id", s.id);
    const msg = resolution === "teacher" ? "Dispute resolved in teacher's favor." : resolution === "learner" ? "Dispute resolved in learner's favor." : "Dispute split equally.";
    try {
      await supabase.from("notifications").insert([
        { user_id: s.teacher_id, type: "dispute", title: "⚖️ Dispute Resolved", body: `${msg} ${resolveNote}`, link: "/sessions" },
        { user_id: s.learner_id, type: "dispute", title: "⚖️ Dispute Resolved", body: `${msg} ${resolveNote}`, link: "/sessions" },
      ]);
    } catch (_) {}
    showToast(msg);
    setResolveModal(null); setResolveNote(""); setResolution("teacher");
    await loadDisputes(); setActionLoading(null);
  }
  ...
  async function handleRemoveRating(rating: Rating) {
    setActionLoading("rating-" + rating.id);
    await supabase.from("ratings").delete().eq("id", rating.id);
    showToast("Rating removed.");
    await loadRatings(); setActionLoading(null);
  }

  async function handleDismissRating(rating: Rating) {
    setActionLoading("rating-" + rating.id);
    await supabase.from("ratings").update({ is_flagged: false }).eq("id", rating.id);
  ...
  const TABS = [
    ...
    { id: "content",  label: "🛡️ Content",     count: contentItems.length },
    { id: "listings", label: "📋 Flagged",     count: listings.length },
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#0d1117", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ width: 32, height: 32, border: "2px solid #a371f7", borderTopColor: "transparent", borderRadius: "50%", animation: "spin .8s linear infinite" }} />
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: "#0d1117", fontFamily: "'DM Sans', sans-serif", color: "#e6edf3" }}>
      ...
      <div style={{ background: "#161b22", borderBottom: "1px solid #30363d", padding: "0 28px", height: 56, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <span style={{ fontFamily: "'Fraunces', serif", fontSize: 18, fontWeight: 900 }}>
            <span style={{ color: "#2d6a4f" }}>Skill</span><span style={{ color: "#e6edf3" }}>Credit</span>
          </span>
          <div style={{ width: 1, height: 20, background: "#30363d" }} />
          <span style={{ fontSize: 11, fontWeight: 700, color: "#a371f7", background: "#1c1a3b", padding: "3px 10px", borderRadius: 99, letterSpacing: 1 }}>MODERATOR</span>
        </div>
        <div style={{ display: "flex", gap: 4 }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id as any)}
              style={{ padding: "6px 14px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600, fontFamily: "'DM Sans', sans-serif", background: tab === t.id ? "#21262d" : "transparent", color: tab === t.id ? "#e6edf3" : "#8b949e" }}>
              {t.label}
              {t.count > 0 && <span style={{ marginLeft: 5, fontSize: 10, background: tab === t.id ? "#f85149" : "#30363d", color: "#fff", padding: "1px 5px", borderRadius: 99, fontWeight: 800 }}>{t.count}</span>}
            </button>
          ))}
        </div>
        <a href="/dashboard" style={{ fontSize: 12, color: "#8b949e", padding: "5px 12px", borderRadius: 8, border: "1px solid #30363d" }}>← Back to App</a>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 28px 60px" }}>
          ...
          <div style={{ animation: "fadeUp .3s ease" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
              <div>
                ...
                <p style={{ fontSize: 12, color: "#8b949e" }}>Click any item to view full content, comments, and author details.</p>
              </div>
              <button onClick={() => { loadAllContent(); setSelectedContent(null); }} disabled={contentLoading}
                style={{ padding: "7px 14px", borderRadius: 9, border: "1px solid #30363d", background: "#21262d", color: "#8b949e", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
              ...
              {CONTENT_FILTERS.map(type => {
                ...
                return (
                  <button key={type} onClick={() => { setContentFilter(type); setSelectedContent(null); }}
                    style={{ padding: "5px 12px", borderRadius: 99, border: `1px solid ${contentFilter === type ? cfg.color : "#30363d"}`, background: contentFilter === type ? cfg.bg : "transparent", color: contentFilter === type ? cfg.color : "#8b949e", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", gap: 4 }}>
                    {cfg.icon} {cfg.label} <span style={{ fontSize: 10, background: "#30363d", color: "#e6edf3", padding: "1px 5px", borderRadius: 99 }}>{count}</span>
                  </button>
                );
              })}
            </div>
                  ...
                  {filteredContent.length === 0 ? (
                    ...
                    return (
                      <div key={item.id} onClick={() => loadDetail(item)}
                        style={{ background: isSelected ? "#1c2a3a" : "#161b22", border: `1px solid ${isSelected ? "#58a6ff" : "#21262d"}`, borderRadius: 10, padding: "12px 14px", cursor: "pointer", transition: "all .12s" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                          <span style={{ fontSize: 9, fontWeight: 800, padding: "2px 7px", borderRadius: 99, background: cfg.bg, color: cfg.color, letterSpacing: 0.5, whiteSpace: "nowrap" }}>
                            {cfg.icon} {cfg.label.toUpperCase()}
                          </span>
                          {item.status && item.status !== "active" && (
                            <span style={{ fontSize: 9, background: "#450a0a", color: "#f85149", padding: "1px 6px", borderRadius: 99, fontWeight: 700 }}>{item.status}</span>
                          )}
                        ...
                        {item.extra && (
                          <div style={{ fontSize: 10, color: "#8b949e", marginBottom: 3, fontWeight: 600 }}>
                            {item.type === "comment" ? `💬 on: "${item.extra?.slice(0, 40)}…"` : item.extra?.slice(0, 50)}
                          </div>
                        )}
                        <div style={{ fontSize: 12, color: "#c9d1d9", lineHeight: 1.5, marginBottom: 6, wordBreak: "break-word" }}>
                          {item.content?.slice(0, 100)}{(item.content?.length || 0) > 100 ? "…" : ""}
                        </div>
                        {item.author && (
                          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            <Avatar user={item.author} size={18} />
                            <span style={{ fontSize: 10, color: "#8b949e" }}>{item.author.full_name}</span>
                          </div>
                        )}
                      </div>
                    );
                  ...
                  <div style={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 14, overflow: "hidden", maxHeight: "calc(100vh - 280px)", display: "flex", flexDirection: "column" }}>
                    <div style={{ padding: "16px 20px", borderBottom: "1px solid #21262d", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        {(() => { const cfg = CONTENT_TYPE_CONFIG[selectedContent.type]; return (
                          <span style={{ fontSize: 10, fontWeight: 800, padding: "3px 9px", borderRadius: 99, background: cfg.bg, color: cfg.color, letterSpacing: 0.5 }}>
                            {cfg.icon} {cfg.label.toUpperCase()}
                          </span>
                        ); })()}
                        <span style={{ fontSize: 11, color: "#6e7681" }}>
                          {new Date(selectedContent.created_at).toLocaleDateString("en-PH", { month: "long", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" })}
                        </span>
                      </div>
                      <button onClick={() => setSelectedContent(null)} style={{ background: "none", border: "none", color: "#8b949e", fontSize: 18, cursor: "pointer", lineHeight: 1 }}>✕</button>
                    </div>
                    ...
                    <div style={{ flex: 1, overflowY: "auto", padding: "20px" }}>
                      {selectedContent.author && (
                        <div style={{ background: "#21262d", borderRadius: 12, padding: "14px 16px", marginBottom: 18, display: "flex", alignItems: "center", gap: 12 }}>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: 12, color: "#8b949e" }}>@{selectedContent.author.username} · {selectedContent.author.level}</div>
                            {detailProfile && (
                              <div style={{ display: "flex", gap: 12, marginTop: 6 }}>
                                ...
                                <span style={{ fontSize: 11, color: "#6e7681" }}>Joined {new Date(detailProfile.created_at).toLocaleDateString("en-PH", { month: "short", year: "numeric" })}</span>
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <div style={{ marginBottom: 20 }}>
                        {selectedContent.extra && (
                          <div style={{ fontSize: 11, color: "#8b949e", marginBottom: 8, fontWeight: 600 }}>
                            {selectedContent.type === "comment" ? `Commenting on post: "${selectedContent.extra}"` : `Title: ${selectedContent.extra}`}
                          </div>
                        )}
                        <div style={{ fontSize: 14, color: "#e6edf3", lineHeight: 1.7, whiteSpace: "pre-wrap", wordBreak: "break-word", background: "#21262d", borderRadius: 12, padding: "16px" }}>
                          {selectedContent.content || <span style={{ color: "#6e7681", fontStyle: "italic" }}>No content</span>}
                        </div>
                      ...
                      {selectedContent.type === "post" && (
                        <div style={{ marginBottom: 20 }}>
                          <div style={{ fontSize: 12, fontWeight: 700, color: "#8b949e", marginBottom: 10, textTransform: "uppercase", letterSpacing: 0.8 }}>
                            💬 Comments ({detailLoading ? "…" : detailComments.length})
                          </div>
                          {detailLoading ? (
                            <div style={{ textAlign: "center", padding: "20px", color: "#6e7681", fontSize: 12 }}>Loading comments…</div>
                          ) : detailComments.length === 0 ? (
                            <div style={{ textAlign: "center", padding: "16px", color: "#6e7681", fontSize: 12, background: "#21262d", borderRadius: 8 }}>No comments yet</div>
                          ) : detailComments.map((c: any) => (
                            <div key={c.id} style={{ display: "flex", gap: 10, marginBottom: 10, padding: "10px 12px", background: "#21262d", borderRadius: 10 }}>
                              {c.user && <Avatar user={c.user} size={28} />}
                              <div style={{ flex: 1 }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                                  <span style={{ fontSize: 12, fontWeight: 700, color: "#e6edf3" }}>{c.user?.full_name || "Unknown"}</span>
                                  <span style={{ fontSize: 10, color: "#6e7681" }}>@{c.user?.username}</span>
                                  <span style={{ fontSize: 10, color: "#6e7681", marginLeft: "auto" }}>{new Date(c.created_at).toLocaleDateString()}</span>
                                </div>
                                <div style={{ fontSize: 13, color: "#c9d1d9", lineHeight: 1.5 }}>{c.content}</div>
                              </div>
                              <button onClick={() => { setRemoveModal({ ...c, type: "comment", author_id: c.user_id, author: c.user }); setRemoveReason(REMOVAL_REASONS[0]); setRemoveNote(""); }}
                                style={{ padding: "3px 8px", borderRadius: 6, border: "none", background: "#450a0a", color: "#f85149", fontSize: 10, fontWeight: 700, cursor: "pointer", alignSelf: "flex-start", flexShrink: 0, fontFamily: "'DM Sans', sans-serif" }}>
                                Del
                              </button>
                            </div>
                          ))}
                      ...
                      <div style={{ borderTop: "1px solid #21262d", paddingTop: 16, display: "flex", gap: 8, flexWrap: "wrap" }}>
                        ...
                        <button onClick={() => { setRemoveModal(selectedContent); setRemoveReason(REMOVAL_REASONS[0]); setRemoveNote(""); }}
                          style={{ flex: 1, padding: "10px", borderRadius: 10, border: "none", background: "#450a0a", color: "#f85149", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
                          🗑️ Remove Content
                        </button>
                      </div>
                    </div>
        ...
        {tab === "disputes" && (
          <div style={{ animation: "fadeUp .3s ease" }}>
            <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 28, fontWeight: 900, marginBottom: 6 }}>Dispute Resolution</h1>
            <p style={{ fontSize: 13, color: "#8b949e", marginBottom: 20 }}>{disputes.length} open dispute{disputes.length !== 1 ? "s" : ""} waiting for review.</p>
            {disputes.length === 0 ? (
              <div style={{ textAlign: "center", padding: "60px", background: "#161b22", borderRadius: 16, border: "1px solid #30363d", color: "#8b949e" }}>
                <div style={{ fontSize: 40, marginBottom: 12 }}>✅</div>
                <div style={{ fontSize: 16, fontWeight: 700 }}>No open disputes!</div>
              </div>
            ) : disputes.map(d => (
              <div key={d.id} style={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 14, padding: 20, marginBottom: 12, borderLeft: "3px solid #a371f7" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 800, marginBottom: 4 }}>{d.listing?.title || "Untitled Session"}</div>
                    <div style={{ fontSize: 12, color: "#8b949e" }}>{d.credit_amount} cr in escrow · {new Date(d.created_at).toLocaleDateString()}</div>
                  </div>
                  <button onClick={() => { setResolveModal(d); setResolution("teacher"); setResolveNote(""); }}
                    style={{ padding: "8px 18px", borderRadius: 10, border: "none", background: "#1c1a3b", color: "#a371f7", fontSize: 13, fontWeight: 800, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
                    Resolve ⚖️
                  </button>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  {[{ label: "Teacher", user: d.teacher }, { label: "Learner", user: d.learner }].map(({ label, user }) => user && (
                    <div key={label} style={{ background: "#21262d", borderRadius: 10, padding: "12px 14px", display: "flex", alignItems: "center", gap: 10 }}>
                      <Avatar user={user} size={34} />
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 12, fontWeight: 700 }}>{user.full_name}</div>
                        <div style={{ fontSize: 11, color: "#8b949e" }}>@{user.username} · <span style={{ color: "#a371f7", fontWeight: 700 }}>{label}</span></div>
                      </div>
                      <button onClick={() => setWarnModal(user)} style={{ padding: "3px 10px", borderRadius: 7, border: "1px solid #30363d", background: "transparent", color: "#f59e0b", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Warn</button>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {tab === "listings" && (
          <div style={{ animation: "fadeUp .3s ease" }}>
            <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 28, fontWeight: 900, marginBottom: 6 }}>Flagged Listings</h1>
            <p style={{ fontSize: 13, color: "#8b949e", marginBottom: 20 }}>{listings.length} listing{listings.length !== 1 ? "s" : ""} flagged for review.</p>
            {listings.length === 0 ? (
              <div style={{ textAlign: "center", padding: "60px", background: "#161b22", borderRadius: 16, border: "1px solid #30363d", color: "#8b949e" }}>
                <div style={{ fontSize: 40, marginBottom: 12 }}>✅</div>
                <div style={{ fontSize: 16, fontWeight: 700 }}>No flagged listings!</div>
              </div>
            ) : listings.map(l => (
              <div key={l.id} style={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 14, padding: "18px 20px", marginBottom: 10, borderLeft: "3px solid #f59e0b" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#8b949e", marginBottom: 8 }}>{l.description?.slice(0, 120)}…</div>
                    {l.teacher && (
                      <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                        <Avatar user={l.teacher} size={22} />
                        <span style={{ fontSize: 12, color: "#8b949e" }}>{l.teacher.full_name} · @{l.teacher.username}</span>
                      </div>
                    )}
                  </div>
                  <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                    <button onClick={() => handleApproveListing(l)} disabled={!!actionLoading}
                      style={{ padding: "7px 14px", borderRadius: 9, border: "none", background: "#0d4429", color: "#3fb950", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>✓ Approve</button>
                    <button onClick={() => handleRemoveListing(l)} disabled={!!actionLoading}
                      style={{ padding: "7px 14px", borderRadius: 9, border: "none", background: "#450a0a", color: "#f85149", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Remove</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        {tab === "ratings" && (
          <div style={{ animation: "fadeUp .3s ease" }}>
            <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 28, fontWeight: 900, marginBottom: 6 }}>Flagged Ratings</h1>
            <p style={{ fontSize: 13, color: "#8b949e", marginBottom: 20 }}>{ratings.length} rating{ratings.length !== 1 ? "s" : ""} flagged.</p>
            {ratings.length === 0 ? (
              <div style={{ textAlign: "center", padding: "60px", background: "#161b22", borderRadius: 16, border: "1px solid #30363d", color: "#8b949e" }}>
                <div style={{ fontSize: 40, marginBottom: 12 }}>✅</div>
                <div style={{ fontSize: 16, fontWeight: 700 }}>No flagged ratings!</div>
              </div>
            ) : ratings.map(r => (
              <div key={r.id} style={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 14, padding: "18px 20px", marginBottom: 10, borderLeft: "3px solid #f85149" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, color: "#8b949e", marginBottom: 6 }}>
                      From <strong style={{ color: "#e6edf3" }}>{r.rater?.full_name}</strong> → <strong style={{ color: "#e6edf3" }}>{r.rated?.full_name}</strong>
                      <span style={{ marginLeft: 8, color: "#f59e0b" }}>{"★".repeat(r.overall)}{"☆".repeat(5 - r.overall)}</span>
                    </div>
                    {r.review && <div style={{ fontSize: 13, color: "#c9d1d9", background: "#21262d", borderRadius: 9, padding: "10px 14px", marginBottom: 6 }}>"{r.review}"</div>}
                    <div style={{ fontSize: 11, color: "#6e7681" }}>{new Date(r.created_at).toLocaleDateString()}</div>
                  </div>
                  <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                    <button onClick={() => handleDismissRating(r)} disabled={!!actionLoading}
                      style={{ padding: "7px 14px", borderRadius: 9, border: "none", background: "#21262d", color: "#8b949e", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Dismiss</button>
                    <button onClick={() => handleRemoveRating(r)} disabled={!!actionLoading}
                      style={{ padding: "7px 14px", borderRadius: 9, border: "none", background: "#450a0a", color: "#f85149", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Remove</button>
                  </div>
                </div>
              </div>
        ...
        {tab === "reports" && (
          <div style={{ animation: "fadeUp .3s ease" }}>
            <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 28, fontWeight: 900, marginBottom: 6 }}>User Reports</h1>
            <p style={{ fontSize: 13, color: "#8b949e", marginBottom: 20 }}>{reports.length} open report{reports.length !== 1 ? "s" : ""}.</p>
            {reports.length === 0 ? (
              <div style={{ textAlign: "center", padding: "60px", background: "#161b22", borderRadius: 16, border: "1px solid #30363d", color: "#8b949e" }}>
                <div style={{ fontSize: 40, marginBottom: 12 }}>✅</div>
                <div style={{ fontSize: 16, fontWeight: 700 }}>No open reports!</div>
              </div>
            ) : reports.map(r => (
              <div key={r.id} style={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 14, padding: "18px 20px", marginBottom: 10, borderLeft: "3px solid #f85149" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, color: "#8b949e", marginBottom: 6 }}>
                      <strong style={{ color: "#e6edf3" }}>{r.reporter?.full_name}</strong> reported <strong style={{ color: "#f85149" }}>{r.reported?.full_name}</strong>
                    </div>
                    <div style={{ fontSize: 13, color: "#c9d1d9", background: "#21262d", borderRadius: 9, padding: "10px 14px", marginBottom: 6 }}>{r.reason}</div>
                    <div style={{ fontSize: 11, color: "#6e7681" }}>{new Date(r.created_at).toLocaleDateString()}</div>
                  </div>
                  <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                    <button onClick={() => setWarnModal({ id: r.reported_id, full_name: r.reported?.full_name })}
                      style={{ padding: "7px 14px", borderRadius: 9, border: "none", background: "#451a03", color: "#f59e0b", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Warn User</button>
                    <button onClick={() => handleResolveReport(r.id)}
                      style={{ padding: "7px 14px", borderRadius: 9, border: "none", background: "#0d4429", color: "#3fb950", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Resolve</button>
                  </div>
                </div>
              </div>
            ))}
      ...
      {removeModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.82)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 20, animation: "fadeIn .15s ease" }}>
          <div style={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 18, width: "100%", maxWidth: 520, padding: 28, maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
              <div>
                <div style={{ fontSize: 17, fontWeight: 800, color: "#f85149", fontFamily: "'Fraunces', serif" }}>
                  Remove {CONTENT_TYPE_CONFIG[removeModal.type]?.icon} {CONTENT_TYPE_CONFIG[removeModal.type]?.label}
                </div>
                <div style={{ fontSize: 12, color: "#8b949e", marginTop: 3 }}>Content is deleted. User receives a notification with the reason.</div>
              </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {REMOVAL_REASONS.map(reason => (
                  <label key={reason} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderRadius: 8, border: `1px solid ${removeReason === reason ? "#f85149" : "#30363d"}`, background: removeReason === reason ? "#1a0a0a" : "#21262d", cursor: "pointer" }}>
                    <input type="radio" value={reason} checked={removeReason === reason} onChange={() => setRemoveReason(reason)} style={{ accentColor: "#f85149" }} />
                    <span style={{ fontSize: 12, color: removeReason === reason ? "#f85149" : "#8b949e", fontWeight: removeReason === reason ? 700 : 400 }}>{reason}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={handleRemoveContent} disabled={!!actionLoading}
                style={{ flex: 2, padding: "11px", borderRadius: 10, border: "none", background: "#450a0a", color: "#f85149", fontWeight: 800, fontSize: 14, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {resolveModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 20, animation: "fadeIn .15s ease" }}>
          <div style={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 18, width: "100%", maxWidth: 500, padding: 28 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
              <div>
                <div style={{ fontSize: 18, fontWeight: 800, fontFamily: "'Fraunces', serif" }}>Resolve Dispute</div>
                <div style={{ fontSize: 12, color: "#8b949e", marginTop: 2 }}>{resolveModal.listing?.title} · {resolveModal.credit_amount} cr</div>
              </div>
              <button onClick={() => setResolveModal(null)} style={{ background: "none", border: "none", color: "#8b949e", fontSize: 20, cursor: "pointer" }}>✕</button>
            </div>
            <div style={{ marginBottom: 18 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: "#8b949e", display: "block", marginBottom: 10, textTransform: "uppercase", letterSpacing: 0.8 }}>Award credits to</label>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[
                  ...
                  <label key={o.val} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", borderRadius: 10, border: `1px solid ${resolution === o.val ? "#a371f7" : "#30363d"}`, background: resolution === o.val ? "#1c1a3b" : "#21262d", cursor: "pointer" }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: "#8b949e" }}>{o.sub}</div>
                    </div>
                  </label>
                ))}
              </div>
            ...
            <div style={{ marginBottom: 18 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: "#8b949e", display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.8 }}>Note to both parties</label>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setResolveModal(null)} style={{ flex: 1, padding: "11px", borderRadius: 10, border: "1px solid #30363d", background: "transparent", color: "#8b949e", fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Cancel</button>
              <button onClick={handleResolveDispute} disabled={!!actionLoading}
                style={{ flex: 2, padding: "11px", borderRadius: 10, border: "none", background: "#1c1a3b", color: "#a371f7", fontWeight: 800, fontSize: 14, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {warnModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 20, animation: "fadeIn .15s ease" }}>
          <div style={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 18, width: "100%", maxWidth: 400, padding: 26 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
              <div style={{ fontSize: 16, fontWeight: 800 }}>⚠️ Warn {warnModal.full_name}</div>
              <button onClick={() => setWarnModal(null)} style={{ background: "none", border: "none", color: "#8b949e", fontSize: 20, cursor: "pointer" }}>✕</button>
            </div>
            <textarea value={warnMsg} onChange={e => setWarnMsg(e.target.value)} placeholder="Warning message sent to the user as a notification..."
              style={{ width: "100%", minHeight: 100, padding: "10px 12px", borderRadius: 9, border: "1px solid #30363d", background: "#0d1117", color: "#e6edf3", fontSize: 13, fontFamily: "'DM Sans', sans-serif", resize: "vertical", marginBottom: 14 }} />
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setWarnModal(null)} style={{ flex: 1, padding: "10px", borderRadius: 10, border: "1px solid #30363d", background: "transparent", color: "#8b949e", fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Cancel</button>
              <button onClick={handleWarnUser} disabled={!warnMsg || !!actionLoading}
                style={{ flex: 2, padding: "10px", borderRadius: 10, border: "none", background: warnMsg ? "#451a03" : "#21262d", color: warnMsg ? "#f59e0b" : "#8b949e", fontWeight: 800, cursor: warnMsg ? "pointer" : "default", fontFamily: "'DM Sans', sans-serif" }}>
