...
// ── Badge tier: XP + Sessions only (rating lives in Reputation Score) ─────────
...
// ─── AVATAR ───────────────────────────────────────────────────────────────────
function Avatar({ name, level, avatarUrl, size = 44, radius = 12 }: {
  name: string; level: string; avatarUrl?: string | null; size?: number; radius?: number;
}) {
          ...
          <>
            <div style={{ padding:"0 20px 16px", borderBottom:"1.5px solid #f0ece4", flexShrink:0 }}>
              <div style={{ marginTop:-28, marginBottom:12, display:"flex", alignItems:"flex-end", justifyContent:"space-between" }}>
                <Avatar name={user.full_name} level={user.level} avatarUrl={user.avatar_url} size={62} radius={16} />
                {!isOwn ? (
    ...
    <div style={{ minHeight:"100vh", background:"#f7f5f0", fontFamily:"'DM Sans',sans-serif" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; } a { text-decoration:none; }
        ...
        .top-card { transition:all 0.2s ease; cursor:pointer; }
        ...
        select:focus, input:focus { border-color:#2d6a4f !important; box-shadow:0 0 0 3px rgba(45,106,79,0.08) !important; }
      ...
      <nav style={{ background:"rgba(255,255,255,0.97)", backdropFilter:"blur(12px)", borderBottom:"1.5px solid #e8e2d9", padding:"0 28px", height:58, display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:100 }}>
        ...
        <a href="/profile" style={{ display:"flex", alignItems:"center", gap:8, padding:"5px 12px 5px 6px", borderRadius:999, background:"#f5f0e8", border:"1.5px solid #e8e2d9" }}>
          <Avatar name={currentUser?.full_name||""} level={currentUser?.level||"Seedling"} avatarUrl={currentUser?.avatar_url} size={28} radius={999} />
        ...
        <div style={{ display:"flex", gap:10, marginBottom:24, flexWrap:"wrap", animation:"fadeUp 0.4s 0.1s ease both" }}>
          <div style={{ position:"relative", flex:1, minWidth:220 }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, username, location…"
              style={{ width:"100%", padding:"11px 14px 11px 40px", borderRadius:12, border:"1.5px solid #e8e2d9", background:"#fff", color:"#1a1a1a", fontSize:13, fontFamily:"'DM Sans',sans-serif", transition:"all 0.15s" }} />
          </div>
          {[
            ...
            <select key={i} value={f.val} onChange={e => f.set(e.target.value as any)}
              style={{ padding:"11px 14px", borderRadius:12, border:"1.5px solid #e8e2d9", background:"#fff", color:"#555", fontSize:13, fontFamily:"'DM Sans',sans-serif", cursor:"pointer", transition:"all 0.15s" }}>
              ...
              {top3.map((user, idx) => {
                ...
                return (
                  <div key={user.id} className="top-card" onClick={() => setSelectedUserId(user.id)}
                    style={{ background:"#fff", borderRadius:20, border: idx===0 ? `2px solid ${lvl.bg}33` : "1.5px solid #e8e2d9", overflow:"hidden",
                      boxShadow: idx===0 ? `0 8px 32px ${lvl.glow}` : "0 2px 12px rgba(0,0,0,0.05)",
                      animation:`fadeUp 0.5s ${idx*0.08}s ease both`, position:"relative" }}>
                    ...
                    <div style={{ padding:"22px 18px 18px" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:14 }}>
                        <Avatar name={user.full_name} level={user.level} avatarUrl={user.avatar_url} size={54} radius={15} />
              ...
              {(isFiltering ? filtered : rest).map((user, idx) => {
                ...
                return (
                  <div key={user.id} className="user-card" onClick={() => setSelectedUserId(user.id)}
                    style={{ background:"#fff", borderRadius:16, border:"1.5px solid #e8e2d9", overflow:"hidden", animation:`fadeUp 0.4s ${(idx%8)*0.04}s ease both`, boxShadow:"0 2px 8px rgba(0,0,0,0.04)" }}>

                    <div style={{ height:3, background:`linear-gradient(90deg,${lvl.bg},${lvl.bg}33)` }} />

                    <div style={{ padding:"16px" }}>
                      <div style={{ display:"flex", alignItems:"flex-start", gap:11, marginBottom:12 }}>
                        <Avatar name={user.full_name} level={user.level} avatarUrl={user.avatar_url} size={46} radius={13} />
                        <div style={{ flex:1, minWidth:0 }}>
